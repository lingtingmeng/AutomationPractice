...
public class C8897_TermsAndConditions extends WebDriverBase{
	 ...
	 * @throws Exception
	 */
	public void termsPageHtml() throws Exception
	{		
		homeUrlStr = ksLoadInfo.getHomeURL();
		termsUrlStr = homeUrlStr.concat("terms");
		...
		baseFileLocStr = baseFileLocParentStr.concat("TermsAndConditionsHTML.txt");
		testFileLocStr = testFileLocParentStr.concat("TermsAndConditionsHTML_Test.txt");
		
		KSU.testMsgOut("Test Case C8897: Terms & Conditions - " + termsUrlStr);
		
		theDriver.get(termsUrlStr);
		WebActionUtil.maxWindow();
		Thread.sleep(3000);
		
		if(theDriver.getCurrentUrl().equals(termsUrlStr)) {
		...
		KSU.testMsgOut("Test Case C8897 (Done): Terms & Conditions");
