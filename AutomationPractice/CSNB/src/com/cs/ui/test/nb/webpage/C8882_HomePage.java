...
public class C8882_HomePage extends WebDriverBase{
	 ...
	 * @throws Exception
	 */
	public void homePageHtml() throws Exception
	{		
		...
		baseFileLocStr = baseFileLocParentStr.concat("HomePageHTML.txt");
		testFileLocStr = testFileLocParentStr.concat("HomePageHTML_Test.txt");
		
		KSU.testMsgOut("Test Case C8882: Home Page - " + homeUrlStr);
		
		theDriver.get(homeUrlStr);
		WebActionUtil.maxWindow();
		Thread.sleep(3000);
		
		if(theDriver.getCurrentUrl().equals(homeUrlStr)) {
		...
		KSU.testMsgOut("Test Case C8882 (Done): Home Page");
