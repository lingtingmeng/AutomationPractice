...
public class C8893_Contact extends WebDriverBase{
	 ...
	 * @throws Exception
	 */
	public void contactPageHtml() throws Exception
	{		
		homeUrlStr = ksLoadInfo.getHomeURL();
		contactUrlStr = homeUrlStr.concat("contact");
		...
		baseFileLocStr = baseFileLocParentStr.concat("ContactPageHTML.txt");
		testFileLocStr = testFileLocParentStr.concat("ContactPageHTML_Test.txt");
		
		KSU.testMsgOut("Test Case C8893: Contact Page - " + contactUrlStr);
		
		theDriver.get(contactUrlStr);
		WebActionUtil.maxWindow();
		Thread.sleep(3000);
		
		if(theDriver.getCurrentUrl().equals(contactUrlStr)) {
		...
		KSU.testMsgOut("Test Case C8893 (Done): Contact Page");
