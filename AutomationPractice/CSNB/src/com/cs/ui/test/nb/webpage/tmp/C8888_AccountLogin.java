...
public class C8888_AccountLogin extends WebDriverBase{
	
	private static String loginURLStr, 
	pageHtmlBody, 
	 ...
	 * @throws Exception
	 */
	public void accountLoginHtml() throws Exception
	{		
		...
		baseFileLocStr = baseFileLocParentStr.concat("AccountLoginHTML.txt");
		testFileLocStr = testFileLocParentStr.concat("AccountLoginHTML_Test.txt");
		
		KSU.testMsgOut("Test Case C8888: Account Login - " + loginURLStr);
		
		theDriver.get(loginURLStr);
		WebActionUtil.maxWindow();
		Thread.sleep(3000);
		
		if(theDriver.getCurrentUrl().equals(loginURLStr)) {
		...
		KSU.testMsgOut("Test Case C8888 (Done): Account Login");
