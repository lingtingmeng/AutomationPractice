...
public class NBReportListener extends NBReportListenerAdaptor {

	private static final Logger L = Logger.getLogger(NBReportListener.class);
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(
				outdir, "coinsquare-nb-report.html"))));
	}
	...
	protected void generateMethodDetailReport(List<ISuite> suites) {
		  ...
		  m_methodIndex = 0; for (ISuite suite : suites) { Map<String,
		  ISuiteResult> r = suite.getResults(); for (ISuiteResult r2 :
		  r.values()) { ITestContext testContext = r2.getTestContext(); if
		  (r.values().size() > 0) { m_out.println("<h1>" +
		  testContext.getName() + "</h1>"); }
	...
	private void startResultSummaryTable(String style) {
		...
		m_out.println("<tr><th>Class</th>"
				+ "<th>Test/Method</th><th># of<br/>Scenarios</th><th>Start</th><th>Time<br/>(ms)</th></tr>");
		m_row = 0;
	}
	...
	private void resultDetail(IResultMap tests) {
		 for (ITestResult result : tests.getAllResults()) { ITestNGMethod
		 method = result.getMethod(); m_methodIndex++; String cname =
		 method.getTestClass().getName(); m_out.println("<h2 id=\"m" +
		 m_methodIndex + "\">" + cname + ":" + method.getMethodName() +
		 "</h2>"); Set<ITestResult> resultSet = tests.getResults(method);
		 generateForResult(result, method, resultSet.size()); m_out.println(
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				summaryCell((overview.getStartDate().toString()), true);
				m_out.println("</td>");
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println(".passedodd td {background-color: #58EE92}");
		out.println(".passedeven td {background-color: #58EE92}");
		out.println(".skippedodd td {background-color: #F9E4BC}");
		out.println(".skippedodd td {background-color: #F9E4BC}");
		out.println(".failedodd td,.numi_attn {background-color: #FF0033}");
		out.println(".failedeven td,.stripe .numi_attn {background-color: #FF0033}");
